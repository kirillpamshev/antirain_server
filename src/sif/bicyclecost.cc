...
constexpr double R = 6371000.0; // Средний радиус Земли
constexpr double RAD = PI / 180.0;
double harvesin(const PointLL& point1, const PointLL& point2) {
  // Вычисление расстояния по формуле Харвесина
  double f1 = point1.lat() * RAD;
  double f2 = point2.lat() * RAD;
  double l1 = point1.lng() * RAD;
  double l2 = point2.lng() * RAD;
  double df = f2 - f1;
  double dl = l2 - l1;
  double a = std::sin(df / 2) * std::sin(df / 2) +
...
struct geo_point midpoint(const PointLL& point1, const PointLL& point2) {
  double f1 = point1.lat() * RAD;
  double f2 = point2.lat() * RAD;
  double l1 = point1.lng() * RAD;
  double l2 = point2.lng() * RAD;
  double Bx = std::cos(f2) * std::cos(l2 - l1);
  double By = std::cos(f2) * std::sin(l2 - l1);
  double lat3 = std::atan2(std::sin(f1) + std::sin(f2),
                           std::sqrt((std::cos(f1) + Bx) * (std::cos(f1) + Bx) + By * By));
  double lon3 = l1 + std::atan2(By, std::cos(f1) + Bx);
  struct geo_point mid = {static_cast<float>(lat3 * (1 / RAD)), static_cast<float>(lon3 * (1 / RAD))};
  return mid;
}

class BicycleWhCost : public BicycleCost {
public:
  BicycleWhCost(const CostingOptions& costing_options) : BicycleCost(costing_options) {
...
private:
  ...
  double weather_cost(const std::vector<midgard::PointLL>& geo_vector) const {
    int sock = 0;
    if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
      LOG_ERROR("\n Socket creation error \n");
      LOG_ERROR(std::to_string(errno));
      return 1.0;
    }
    ...
    uint16_t geo_size = geo_vector.size();
    uint16_t line_size = geo_size - 1;
    // LOG_INFO(std::to_string(geo_size));
    geo_point* mid_point_arr = new geo_point[line_size];
    double* line_dist_arr = new double[line_size];
    for (size_t i = 0; i < line_size; i++) {
      auto point1 = geo_vector[i];
      auto point2 = geo_vector[i + 1];
      mid_point_arr[i] = midpoint(point1, point2);
      line_dist_arr[i] = harvesin(point1, point2);
      // LOG_INFO(std::to_string(p_point[i].lat));
      // LOG_INFO(std::to_string(p_point[i].lng));
    }
    if (send(sock, &line_size, sizeof(line_size), 0) < 0) {
      LOG_ERROR("\nSend Failed \n");
      LOG_ERROR(std::to_string(errno));
      close(sock);
      delete [] mid_point_arr;
      delete [] line_dist_arr;
      return 1.0;
    }
    if (send(sock, mid_point_arr, line_size * sizeof(geo_point), 0) < 0) {
      LOG_ERROR("\nSend Failed \n");
      LOG_ERROR(std::to_string(errno));
      close(sock);
      delete [] mid_point_arr;
    ...
    delete [] mid_point_arr;
    uint8_t* res_arr = new uint8_t[line_size];
    if (read(sock, res_arr, line_size * sizeof(uint8_t)) < 0) {
      LOG_ERROR("\nRecieve Failed \n");
      LOG_ERROR(std::to_string(errno));
      close(sock);
      delete [] line_dist_arr;
    ...
    double s_dist = 0.0;
    double s_weight_dist = 0.0;
    for (size_t i = 0; i < line_size; i++) {
      s_dist += line_dist_arr[i];
      s_weight_dist += (line_dist_arr[i] * static_cast<double>(res_arr[i]));
    }
    delete [] line_dist_arr;
    delete [] res_arr;
    double res = s_weight_dist / s_dist;
    LOG_INFO(std::to_string(res));
    close(sock);
    return res;
  }
};
...
// Check if access is allowed on the specified edge.
bool BicycleWhCost::Allowed(const baldr::DirectedEdge* edge,
                            const bool is_dest,
                            const EdgeLabel& pred,
                            const graph_tile_ptr& tile,
                            const baldr::GraphId& edgeid,
  ...
  return BicycleCost::Allowed(edge, is_dest, pred, tile, edgeid, current_time, tz_index,
                              restriction_idx);
}

// Checks if access is allowed for an edge on the reverse path (from
// destination towards origin). Both opposing edges are provided.
bool BicycleWhCost::AllowedReverse(const baldr::DirectedEdge* edge,
                                   const EdgeLabel& pred,
                                   const baldr::DirectedEdge* opp_edge,
                                   const graph_tile_ptr& tile,
                                   const baldr::GraphId& opp_edgeid,
                                   ...
                                   const uint32_t tz_index,
                                   uint8_t& restriction_idx) const {
  return BicycleCost::AllowedReverse(edge, pred, opp_edge, tile, opp_edgeid, current_time, tz_index,
                                     restriction_idx);
}

Cost BicycleWhCost::EdgeCost(const baldr::DirectedEdge* edge,
                             const graph_tile_ptr& tile,
                             const uint32_t seconds,
                             uint8_t& flow_sources) const {
  auto cost = BicycleCost::EdgeCost(edge, tile, seconds, flow_sources);
  auto edgeinfo = tile->edgeinfo(edge);
  const std::vector<midgard::PointLL>& geo_vector = edgeinfo.shape();
  float weather_coeff = weather_cost(geo_vector);
  cost.cost *= weather_coeff;
  return cost;
}

cost_ptr_t CreateBicycleWHCost(const CostingOptions& costing_options) {
  return std::make_shared<BicycleWhCost>(costing_options);
}
